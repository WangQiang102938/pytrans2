...
class TesseractOCR(PipelineNode):
    ...
    def get_port_keys(self, input_port=True) -> List[str]:
        return [PortEnum.IN_IMAGE.value] if input_port else [PortEnum.OUT_TEXT.value]
    ...
    def process_capnode(
        ...
        node.working_doc.get_memo_with_update(
            self.uuid.hex, PortEnum.IN_IMAGE.value, node.uuid.bytes, raw_val=pickle.dumps(img)
        )
        lang_combo = self.widget.lang_select_combo
        if self.widget.ignore_image.isChecked() and node.node_type == node.Type.IMAGE:
            ocr_result = [[""]]
        else:
            ocr_result = pytesseract.image_to_string(img, lang=lang_combo.currentData())
        node.working_doc.get_memo_with_update(
            self.uuid.hex,
            PortEnum.OUT_TEXT.value,
            node.uuid.bytes,
            raw_val=pickle.dumps(ocr_result),
        )
        return True
...
class TesseractWidget(QFrame):
    def bind(self):
        ...
        self.lang_select_combo = (
            qt_utils.FormItem(self)
            .setup("OCR Lang", self.main_layout)
        ...
        self.lang_select_combo.setStyleSheet("QComboBox{combobox-popup:0;}")
