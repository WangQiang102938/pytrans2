...
class StandardCrop(PipelineNode):
    ...
    def process_capnode(
        ...
        node_rect = preview_utils.map_rect_from_ratio(
            QRectF(
                visual_memo.left, visual_memo.top, visual_memo.right, visual_memo.bottom
            ),
    ...
    def get_output(self, node: CaptureNode, key: str):
        try:
            orm_ins = node.working_doc.sync_memo(self.uuid.hex, key, node.uuid)
