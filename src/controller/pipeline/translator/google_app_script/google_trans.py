...
class GoogleTranslateWidget(QFrame):
    def bind(self):
        ...
        self.api_key_edit, self.api_con = (
            FormItem(self)
            .setup("Api Key", self.main_layout, True)
        ...
        self.api_load_btn = self.api_con.add_content(QPushButton("L"))
        self.api_save_btn = self.api_con.add_content(QPushButton("S"))

        self.char_limit_edit = (
            FormItem(self)
            .setup("Max chars size", self.main_layout)
        ...
        self.src_lang_combo = (
            FormItem(self)
            .setup("Source Language", self.main_layout)
            .add_content(QComboBox())
        )

        self.tar_lang_combo = (
            FormItem(self)
            .setup("Target Language", self.main_layout)
            .add_content(QComboBox())
        )

        self.print_output = (
            FormItem(self)
            .setup("Print result to console", self.main_layout)
...
class GoogleTranslate(PipelineNode):
    ...
    def __init__(self, pipe_hub: PipelineHub) -> None:
        ...
        self.model_hub = self.pipe_hub.ctrl_hub.main.model_hub

        self.option_widget.api_load_btn.clicked.connect(self.load_config)
        self.option_widget.api_save_btn.clicked.connect(self.save_config)
    ...
    def process_end(self):
        ...
        for sent, val in self.text2trans.items():
            if (
                working_queue.__len__() - 2
            ) * 2 + count + sent.__len__() > self.option_widget.char_limit_edit.value():
        ...
        for capnode, origin_sections in self.node2text_dict.items():
            ...
            for origin_sents in origin_sections:
                ...
                txt_out.append(trans_sents)
            if self.option_widget.print_output.isChecked():
                print(txt_out)
            capnode.working_doc.get_memo_with_update(
                self.uuid.hex,
                PortEnum.OUT_TEXT.value,
                capnode.uuid.bytes,
                raw_val=pickle.dumps(txt_out),
            )
    ...
    def get_port_keys(self, input_port=True) -> List[str]:
        return [PortEnum.IN_TEXT.value] if input_port else [PortEnum.OUT_TEXT.value]

    def load_config(self):
        orm = self.model_hub.get_config_with_sync(self.uuid.hex, "api_key")
        if orm != None:
            self.option_widget.api_key_edit.setText(orm.str_val)

    def save_config(self):
        self.model_hub.get_config_with_sync(
            self.uuid.hex, "api_key", val_str=self.option_widget.api_key_edit.text()
        )
