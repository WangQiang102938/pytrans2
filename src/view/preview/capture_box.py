...
class CaptureBoxItem(QGraphicsRectItem):
    def bind(self, preview_hub: 'PreviewHub', node: CaptureNode):
        ...
        self.setRect(preview_utils.map_rect_from_ratio(
            QRectF(
    ...
    def itemChange(self, change: QGraphicsItem.GraphicsItemChange, value: Any) -> Any:
        ...
        if(self.isSelected()):
            if(change == self.GraphicsItemChange.ItemPositionChange):
                ...
                x = min(max(0-box_rect.left(), offset_pos.x()), page_rect.right()-box_rect.right())
                y = min(max(0-box_rect.top(), offset_pos.y()), page_rect.bottom()-box_rect.bottom())
    ...
    def resize(self, anchor_pos: QPointF, diagnal_pos: QPointF, start_pos: QPointF, end_pos: QPointF):
        self.setRect(
            preview_utils.limit_rect(
                preview_utils.rect_from_2point(diagnal_pos, anchor_pos-start_pos+end_pos),
                QRectF(self.preview_hub.page_item.pixmap().rect()), offset_pos=self.pos()
            )
        )
        for item in self.resize_icons:
    ...
    def update_box_preview(self, mount=False, unmount=False):
        if mount:
            ...
            return
        box_rect = self.rect()
        box_rect = QRectF(box_rect.topLeft()+self.pos(), box_rect.bottomRight()+self.pos())
        self.preview_item.setPixmap(self.preview_hub.page_item.pixmap().copy(box_rect.toRect()))
        self.preview_hub.capture_preview_scene.setSceneRect(self.preview_item.boundingRect())
...
class ResizeIconItem(QGraphicsEllipseItem):
    ...
    def mouseMoveEvent(self, event: QGraphicsSceneMouseEvent) -> None:
        self.end_pos = preview_utils.limit_point(event.scenePos(), QRectF(
            self.box.preview_hub.page_item.pixmap().rect()))
