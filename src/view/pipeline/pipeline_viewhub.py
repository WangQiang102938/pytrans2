...
class PipelineViewhub(ViewController):
    def __init__(self, view_hub: ViewHub) -> None:
        ...
        self.ui.PipeAddBtn.clicked.connect(
            lambda x: self.pipeline_edit(PipelineEditFlag.ADD_NODE)
        )
        self.ui.PipeUpBtn.clicked.connect(
            lambda x: self.pipeline_edit(PipelineEditFlag.BRING_UP)
        )
        self.ui.PipeDownBtn.clicked.connect(
            lambda x: self.pipeline_edit(PipelineEditFlag.BRING_DOWN)
        )
        self.ui.PipeRemoveBtn.clicked.connect(
            lambda x: self.pipeline_edit(PipelineEditFlag.REMOVE_NODE)
        )
    ...
    def pipeline_edit(self, flag: "PipelineEditFlag"):
        ...
        def index_from_item(item: QListWidgetItem):
            ...
            return (
                -1
                if ins not in pipeline_node_ins_list
                else pipeline_node_ins_list.index(ins)
            )
